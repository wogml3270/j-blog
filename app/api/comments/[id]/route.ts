...
import { deleteCommentByAuthor, updateCommentByAuthor } from "@/lib/comments/repository";
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  const result = await updateCommentByAuthor({
    commentId: id,
...
export async function DELETE(_: Request, context: RouteContext) {
  ...
  const result = await deleteCommentByAuthor({
    commentId: id,
    authorUserId: user.id,
  });
